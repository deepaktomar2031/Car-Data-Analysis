const averageListingSelling = (listingsArray) => {
  let responseObject = listingsArray.reduce(groupFunction, {});

  let finalResponse = [];

  Object.keys(responseObject).forEach((seller_type) => {
    const average_price = averageFunction(
      responseObject[seller_type].totalPrice,
      responseObject[seller_type].numberOfCars
    );
    finalResponse.push({
...
module.exports = averageListingSelling;
